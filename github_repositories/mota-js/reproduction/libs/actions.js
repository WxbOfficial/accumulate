...
export default class Actions {
	 ...
	 */
	registerAction(action, name, func, priority) {
		if (!name || !func)
			return;
		// 将onclick视为ondown处理
	...
	_sys_keyDown_lockControl(keyCode) {
	...
	////// 根据放开键的code来执行一系列操作 //////
	keyUp(keyCode, altKey, fromReplay) {
		this.doRegisteredAction('keyUp', keyCode, altKey, fromReplay);
	}
	_sys_keyUp_replay(keyCode, altKey, fromReplay) {
		if (!fromReplay && this._checkReplaying()) return true;
	}
	_sys_keyUp_lockControl(keyCode, altKey) {
		if (!core.status.lockControl) return false;
	...
	_sys_ondown_lockControl(x, y, px, py) {
	...
	_sys_longClick_lockControl(x, y, px, py) {
		if (!core.status.lockControl) return false;
		if (core.status.event.id == 'text') {
			core.drawText();
			return true;
	...
	_clickSwitchs_display_setSize(delta) {
	...
	_clickSwitchs_display_enableHDCanvas() {
	...
	_clickSwitchs_action_moveSpeed(delta) {
	...
	_clickSwitchs_action_floorChangeTime(delta) {
	...
	_clickSyncSave_replay() {
		core.ui._drawReplay();
	}
	////// 同步存档界面时，放开某个键的操作 //////
	_keyUpSyncSave(keycode) {
		if (keycode == 27 || keycode == 88) {
			core.status.event.selection = 4;
			core.playSound('取消');
			core.ui._drawSettings();
