...
export default class Enemys {
	...
	_calSpecialContent(enemy, content) {
	...
	////// 获得某个点上某个怪物的某项属性 //////
	getEnemyValue(enemy, name, x, y, floorId) {
	...
	////// 接下来N个临界值和临界减伤计算 //////
	nextCriticals(enemy, number, x, y, floorId) {
		if (typeof enemy == 'string') enemy = core.material.enemys[enemy];
		number = number || 1;
	...
	/// 未破防临界采用二分计算
	_nextCriticals_overAtk(enemy, x, y, floorId) {
	...
	_nextCriticals_special(enemy, number, x, y, floorId) {
		if (this.hasSpecial(enemy.special, 10) || this.hasSpecial(enemy.special, 3))
			return []; // 模仿or坚固临界
		return null;
	}
	_nextCriticals_useLoop(enemy, info, number, x, y, floorId) {
	...
	_nextCriticals_useBinarySearch(enemy, info, number, x, y, floorId) {
		var mon_hp = info.mon_hp, hero_atk = core.status.hero.atk, mon_def = info.mon_def, pre = info.damage;
		var list = [];
		var start_atk = hero_atk;
		if (info.__over__) {
	...
	_nextCriticals_useTurn(enemy, info, number, x, y, floorId) {
	...
	////// N防减伤计算 //////
	getDefDamage(enemy, k, x, y, floorId) {
		if (typeof enemy == 'string') enemy = core.material.enemys[enemy];
		k = k || 1;
	...
	////// 获得在某个勇士属性下怪物伤害 //////
	getDamage(enemy, x, y, floorId) {
		return this._getDamage(enemy, null, x, y, floorId);
	}
	_getDamage(enemy, hero, x, y, floorId) {
	...
	_getCurrentEnemys_getEnemy(enemyId) {
	...
	_getCurrentEnemys_addEnemy(enemyId, enemys, used, x, y, floorId) {
