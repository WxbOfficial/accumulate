...
export default class Maps {
	...
	_drawThumbnail_drawTempCanvas(floorId, blocks, options) {
		...
		if (options.all) {
			...
			var offsetX = core.clamp(centerX - core._HALF_WIDTH_, 0, width - core._WIDTH_), offsetY = core.clamp(centerY - core._HALF_HEIGHT_, 0, height - core._HEIGHT_);
	...
	_drawThumbnail_drawToTarget(floorId, options) {
		...
		if (options.all) {
			...
			if (options.v2) {
				...
				var offsetX = core.clamp(centerX - hw, 0, width - W), offsetY = core.clamp(centerY - hh, 0, height - H);
				if (options.noHD) {
	...
	////// 获得某个点的block //////
	getBlock(x, y, floorId, showDisable) {
		floorId = floorId || core.status.floorId;
		if (!floorId) return null;
		core.extractBlocks(floorId);
	...
	////// 获得某个点的blockId //////
	getBlockId(x, y, floorId, showDisable) {
