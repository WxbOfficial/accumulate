...
export default class Ui {
	...
	////// 清除地图 //////
	clearMap(name, x, y, width, height) {
		if (name == 'all') {
	...
	////// 在某个canvas上绘制一段文字 //////
	fillText(name, text, x, y, style, font, maxWidth) {
	...
	////// 自适配字体大小
	setFontForMaxWidth(name, text, maxWidth, font) {
		var ctx = this.getContextByName(name);
		if (font) core.setFont(name, font);
	...
	////// 在某个canvas上绘制粗体 //////
	fillBoldText(name, text, x, y, style, strokeStyle, font, maxWidth) {
		var ctx = this.getContextByName(name);
		if (!ctx) return;
	...
	////// 在某个canvas上绘制一个矩形 //////
	fillRect(name, x, y, width, height, style, angle) {
		if (style) core.setFillStyle(name, style);
		var ctx = this.getContextByName(name);
		if (ctx) {
			if (angle) {
	...
	////// 在某个canvas上绘制一个矩形的边框 //////
	strokeRect(name, x, y, width, height, style, lineWidth, angle) {
	...
	////// 在某个canvas上绘制一个圆角矩形 //////
	fillRoundRect(name, x, y, width, height, radius, style, angle) {
		if (style) core.setFillStyle(name, style);
		var ctx = this.getContextByName(name);
		if (ctx) {
			if (angle) {
	...
	strokeRoundRect(name, x, y, width, height, radius, style, lineWidth, angle) {
		if (style) core.setStrokeStyle(name, style);
		if (lineWidth) core.setLineWidth(name, lineWidth);
		var ctx = this.getContextByName(name);
		if (ctx) {
	...
	_roundRect_buildPath(ctx, x, y, width, height, radius) {
		ctx.beginPath();
	...
	_uievent_strokePolygon(data) {
		this._createUIEvent();
		this.strokePolygon('uievent', data.nodes, data.style, data.lineWidth);
	}
	////// 在某个canvas上绘制一个椭圆 //////
	fillEllipse(name, x, y, a, b, angle, style) {
		if (style) core.setFillStyle(name, style);
		var ctx = this.getContextByName(name);
		if (!ctx) return;
		ctx.beginPath();
	...
	fillCircle(name, x, y, r, style) {
		return this.fillEllipse(name, x, y, r, r, 0, style);
	}
	_uievent_fillEllipse(data) {
		this._createUIEvent();
	...
	////// 在某个canvas上绘制一个圆的边框 //////
	strokeEllipse(name, x, y, a, b, angle, style, lineWidth) {
		if (style) core.setStrokeStyle(name, style);
		if (lineWidth) core.setLineWidth(name, lineWidth);
		var ctx = this.getContextByName(name);
		if (!ctx) return;
	...
	strokeCircle(name, x, y, r, style, lineWidth) {
		return this.strokeEllipse(name, x, y, r, r, 0, style, lineWidth);
	}
	_uievent_strokeEllipse(data) {
		this._createUIEvent();
	...
	fillArc(name, x, y, r, start, end, style) {
		if (style) core.setFillStyle(name, style);
		var ctx = this.getContextByName(name);
		if (!ctx) return;
		ctx.beginPath();
	...
	strokeArc(name, x, y, r, start, end, style, lineWidth) {
		if (style) core.setStrokeStyle(name, style);
		if (lineWidth) core.setLineWidth(name, lineWidth);
		var ctx = this.getContextByName(name);
		if (!ctx) return;
	...
	////// 在某个canvas上绘制一条线 //////
	drawLine(name, x1, y1, x2, y2, style, lineWidth) {
		if (style) core.setStrokeStyle(name, style);
	...
	////// 在某个canvas上绘制一个箭头 //////
	drawArrow(name, x1, y1, x2, y2, style, lineWidth) {
	...
	_uievent_setFilter(data) {
		this._createUIEvent();
		this.setFilter('uievent', data);
	}
	////// 计算某段文字的宽度 //////
	calWidth(name, text, font) {
		var ctx = this.getContextByName(name);
		if (ctx) {
			if (font) ctx.font = font;
	...
	////// 字符串自动换行的分割 //////
	splitLines(name, text, maxWidth, font) {
		var ctx = this.getContextByName(name);
		if (!ctx) return [text];
		if (font) core.setFont(name, font);
	...
	////// 绘制一张图片 //////
	drawImage(name, image, x, y, w, h, x1, y1, w1, h1, angle, reverse) {
	...
	drawIcon(name, id, x, y, w, h, frame) {
		frame = frame || 0;
		var ctx = this.getContextByName(name);
		if (!ctx) return;
	...
	_drawText_setContent(contents, callback) {
		// 合并进 insertAction
	...
	_drawSelector(ctx, background, w, h, left, top) {
		left = left || 0;
		top = top || 0;
		// back
	...
	////// 绘制 WindowSkin
	drawWindowSkin(background, ctx, x, y, w, h, direction, px, py) {
	...
	drawBackground(left, top, right, bottom, posInfo) {
		posInfo = posInfo || {};
	...
	_drawWindowSkin_getOpacity() {
		return core.getFlag("__winskin_opacity__", 0.85);
	}
	_drawBackground_drawWindowSkin(background, left, top, right, bottom, position, px, py, ctx) {
		ctx = ctx || 'ui';
	...
	_drawBackground_drawColor(background, left, top, right, bottom, position, px, py, xoffset, yoffset, ctx) {
		ctx = ctx || 'ui';
	...
	////// 计算有效文本框的宽度
	_calTextBoxWidth(ctx, content, min_width, max_width, font) {
		// 无限长度自动换行
	...
	_buildFont(fontSize, bold, italic, font) {
		var textAttribute = core.status.textAttribute || core.initStatus.textAttribute, globalAttribute = core.status.globalAttribute || core.initStatus.globalAttribute;
	...
	_drawTextContent_next(tempCtx, content, config) {
	...
	// 绘制下一个字符
	_drawTextContent_drawChar(tempCtx, content, config, ch) {
		// 标点禁则：不能在行首的标点
		var forbidStart = "）)】》＞﹞>)]»›〕〉}］」｝〗』" + "，。？！：；·…,.?!:;、……~&@#～＆＠＃";
		// 标点禁则：不能在行尾的标点
		var forbidEnd = "（(【《＜﹝<([«‹〔〈{［「｛〖『";
	...
	_drawTextContent_newLine(tempCtx, config) {
		// 计算偏移量
	...
	_drawTextContent_changeColor(tempCtx, content, config) {
		// 检查是不是 []
	...
	_drawTextContent_changeFontSize(tempCtx, content, config) {
		config.index++;
		// 检查是不是 []
		var index = config.index, index2;
		if (content.charAt(index) == '[' && ((index2 = content.indexOf(']', index)) >= 0)) {
	...
	_drawTextContent_changeFont(tempCtx, content, config) {
		config.index++;
		// 检查是不是 []
		var index = config.index, index2;
		if (content.charAt(index) == '[' && ((index2 = content.indexOf(']', index)) >= 0)) {
	...
	_drawTextContent_emptyChar(tempCtx, content, config) {
		config.index++;
		var index = config.index, index2;
		if (content.charAt(index) == '[' && ((index2 = content.indexOf(']', index)) >= 0)) {
			var str = content.substring(index + 1, index2);
	...
	_drawTextContent_drawIcon(tempCtx, content, config) {
		// 绘制一个 \i 效果
		var index = config.index, index2;
	...
	_drawTextBox_drawImages(content, ctx) {
		ctx = ctx || 'ui';
	...
	_drawTextBox_getHorizontalPosition(content, titleInfo, posInfo) {
		...
		var left = 7 + 3 * (core._HALF_WIDTH_ - 6), right = core._PX_ - left, width = right - left, validWidth = width - paddingLeft - paddingRight;
		// 对话框效果：改为动态计算
	...
	_drawTextBox_getVerticalPosition(content, titleInfo, posInfo, validWidth) {
		var textAttribute = core.status.textAttribute || core.initStatus.textAttribute;
	...
	_drawTextBox_drawTitleAndIcon(titleInfo, hPos, vPos, alpha, ctx) {
		ctx = ctx || 'ui';
	...
	_createTextCanvas(content, lineHeight) {
	...
	////// 绘制滚动字幕 //////
	drawScrollText(content, time, lineHeight, callback) {
		content = core.replaceText(content || "");
	...
	_drawScrollText_animate(ctx, time, callback) {
		// 开始绘制到UI上
	...
	////// 绘制一个选项界面 //////
	drawChoices(content, choices, width, ctx) {
	...
	_drawChoices_getHorizontalPosition(titleInfo, choices, width, ctx) {
		ctx = ctx || 'ui';
		// 宽度计算：考虑提示文字和选项的长度
	...
	_drawChoices_getVerticalPosition(titleInfo, choices, hPos) {
	...
	_drawChoices_drawTitle(titleInfo, hPos, vPos, ctx) {
	...
	_drawChoices_drawChoices(choices, isWindowSkin, hPos, vPos, ctx) {
		var hasCtx = ctx != null;
		ctx = ctx || 'ui';
		// 选项
		core.setTextAlign(ctx, 'center');
	...
	////// 绘制一个确认/取消的警告页面 //////
	drawConfirmBox(text, yesCallback, noCallback, ctx) {
		var hasCtx = ctx != null;
		ctx = ctx || 'ui';
		text = core.replaceText(text || "");
	...
	_drawConfirmBox_getRect(contents, ctx) {
	...
	////// 绘制分页 //////
	drawPagination(page, totalPage, y) {
	...
	_drawBook_drawOne(floorId, index, enemy, pageinfo, selected) {
	...
	_drawBook_drawBox(index, enemy, top, pageinfo) {
	...
	_drawBook_drawName(index, enemy, top, left, width) {
		// 绘制第零列（名称和特殊属性）
	...
	_drawBook_drawContent(index, enemy, top, left) {
	...
	_drawBook_drawRow1(index, enemy, top, left, width, position) {
		// 绘制第一行
		core.setTextAlign('ui', 'left');
	...
	_drawBook_drawRow2(index, enemy, top, left, width, position) {
		// 绘制第二行
		core.setTextAlign('ui', 'left');
		var b13 = this._buildFont(13, true), f13 = this._buildFont(13, false);
		var col1 = left, col2 = left + width * 9 / 25, col3 = left + width * 17 / 25;
	...
	_drawBook_drawRow3(index, enemy, top, left, width, position) {
		// 绘制第三行
		core.setTextAlign('ui', 'left');
		var b13 = this._buildFont(13, true), f13 = this._buildFont(13, false);
		var col1 = left, col2 = left + width * 9 / 25, col3 = left + width * 17 / 25;
	...
	_drawBook_drawDamage(index, enemy, offset, position) {
		core.setTextAlign('ui', 'center');
	...
	_drawBookDetail(index) {
		...
		var height = Math.max(this.getTextContentHeight(content, { fontSize: 16, lineHeight: 24, maxWidth: validWidth }) + 58, 80), top = (core._PY_ - height) / 2, bottom = top + height;
	...
	_drawBookDetail_getTexts(enemy, floorId, texts) {
		// --- 原始数值
	...
	_drawBookDetail_origin(enemy, texts) {
	...
	_drawBookDetail_mofang(enemy, texts) {
		// 模仿临界计算器
	...
	_drawBookDetail_mofang_getArray(hp) {
		var arr = [];
		var last = 0, start = 0;
		for (var i = 1; i < hp; i++) {
	...
	_drawBookDetail_vampire(enemy, floorId, texts) {
	...
	_drawBookDetail_drawContent(enemy, content, pos) {
		// 名称
	...
	_drawCenterFly() {
		...
		var offsetX = core.clamp(toX - core._HALF_WIDTH_, 0, core.bigmap.width - core._WIDTH_), offsetY = core.clamp(toY - core._HALF_HEIGHT_, 0, core.bigmap.height - core._HEIGHT_);
	...
	_drawViewMaps_buildData(index, x, y) {
	...
	_drawToolbox_getInfo(index) {
		...
		// 获取物品列表
		var tools = core.getToolboxItems('tools'), constants = core.getToolboxItems('constants');
		// 处理页数
	...
	_drawToolbox_drawBackground() {
		// 绘制
		core.clearMap('ui');
		core.setAlpha('ui', 0.85);
		core.fillRect('ui', 0, 0, core._PX_, core._PY_, '#000000');
	}
	_drawToolbox_drawLine(yoffset, text) {
		core.setFillStyle('ui', '#DDDDDD');
	...
	_drawToolbox_drawDescription(info, max_height) {
		core.setTextAlign('ui', 'left');
	...
	_drawToolbox_drawContent(info, line, items, page, drawCount) {
	...
	_drawEquipbox_description(info, max_height) {
		core.setTextAlign('ui', 'left');
		if (!info.selectId) return;
	...
	_drawEquipbox_getStatusChanged(info, equip, equipType, y) {
	...
	_drawEquipbox_drawStatusChanged(info, y, equip, equipType) {
	...
	_drawEquipbox_drawStatusChanged_draw(text, color, obj) {
	...
	_drawEquipbox_drawEquiped(info, line) {
		core.setTextAlign('ui', 'center');
	...
	_drawSLPanel_draw(page, max_page) {
		// --- 绘制背景
	...
	_drawSLPanel_drawBackground() {
		core.clearMap('ui');
		core.setAlpha('ui', 0.85);
		core.fillRect('ui', 0, 0, core._PX_, core._PY_, '#000000'); // 可改成背景图
		core.setAlpha('ui', 1);
	}
	_drawSLPanel_loadSave(page, callback) {
		var ids = [0];
		for (var i = 1; i <= 5; ++i) {
			var id = 5 * page + i;
	...
	_drawSLPanel_drawRecord(title, data, x, y, size, cho, highLight) {
		var globalAttribute = core.status.globalAttribute || core.initStatus.globalAttribute;
	...
	_drawStatistics_add(floorId, obj, x1, x2, value) {
	...
	_drawStatistics_floorId(floorId, obj) {
	...
	_drawStatistics_enemy(floorId, id, obj) {
	...
	_drawStatistics_items(floorId, floor, id, obj) {
	...
	_drawStatistics_generateText(obj, type, data) {
		var text = type + "地图中：\n";
	...
	////// 动态canvas //////
	////// canvas创建 //////
	createCanvas(name, x, y, width, height, z) {
		// 如果画布已存在则直接调用
	...
	////// canvas重定位 //////
	relocateCanvas(name, x, y, useDelta) {
	...
	////// canvas旋转 //////
	rotateCanvas(name, angle, centerX, centerY) {
		var ctx = core.getContextByName(name);
		if (!ctx) return null;
	...
	////// canvas重置 //////
	resizeCanvas(name, width, height, styleOnly) {
		var ctx = core.getContextByName(name);
