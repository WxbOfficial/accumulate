...
export default class Events {
	...
	////// 游戏结束 //////
	gameOver(ending, fromReplay, norank) {
	...
	_gameOver_confirmUpload(ending, norank) {
		core.ui.closePanel();
	...
	_gameOver_doUpload(username, ending, norank) {
	...
	_gameOver_confirmDownload(ending) {
		core.ui.closePanel();
	...
	registerSystemEvent(type, func) {
		this.systemEvents[type] = func;
	}
	////// 注销一个系统事件 //////
	unregisterSystemEvent(type) {
		delete this.systemEvents[type];
	}
	////// 执行一个系统事件 //////
	doSystemEvent(type, data, callback) {
	...
	_trigger_ignoreChangeFloor(block) {
	...
	_openDoor_check(block, x, y, needKey) {
	...
	_openDoor_animate(block, x, y, callback) {
	...
	afterGetItem(id, x, y, isGentleClick) {
	...
	_getNextItem(direction, noRoute) {
		direction = direction || core.getHeroLoc('direction');
		var nx = core.getHeroLoc('x') + core.utils.scan[direction].x;
		var ny = core.getHeroLoc('y') + core.utils.scan[direction].y;
	...
	_sys_changeFloor(data, callback) {
	...
	////// 转换楼层结束的事件 //////
	afterChangeFloor(floorId) {
		if (main.mode != 'play') return;
		return this.eventdata.afterChangeFloor(floorId);
	}
	////// 是否到达过某个楼层 //////
	hasVisitedFloor(floorId) {
	...
	insertAction(action, x, y, callback, addToLast) {
	...
	insertCommonEvent(name, args, x, y, callback, addToLast) {
		...
		core.status.autoEvents.forEach(function (autoEvent) {
			...
			if (x == null && y == null) {
				event = [
					...
					{
						"type": "function", "function": "function() { core.autoEventExecuting('" + symbol + "', false); }"
					}
				];
			} else {
				event = [
					...
					{ "type": "dowhile", "condition": "false", "data": autoEvent.data },
					{
						"type": "function", "function": "function() { core.popEventLoc(); core.autoEventExecuting('" + symbol + "', false); }"
					}
				];
			}
	...
	autoEventExecuting(symbol, value) {
	...
	autoEventExecuted(symbol, value) {
	...
	__precompile_text(text) {
		if (typeof text != 'string') return text;
		return text.replace(/\${(.*?)}/g, function (word, value) {
			return "${" + core.replaceValue(value) + "}";
		});
	}
	__precompile_array(value) {
	...
	__action_getLoc(loc, x, y, prefix) {
		if (loc) {
	...
	_action_moveTextBox(data, x, y, prefix) {
		if (this.__action_checkReplaying()) return;
	...
	_action_setBlock(data, x, y, prefix) {
		data.loc = this.__action_getLoc2D(data.loc, x, y, prefix);
	...
	_action_setBlockOpacity(data, x, y, prefix) {
		data.loc = this.__action_getLoc2D(data.loc, x, y, prefix);
		if (data.time > 0 && data.floorId == core.status.floorId) {
	...
	_action_setBlockFilter(data, x, y, prefix) {
		data.loc = this.__action_getLoc2D(data.loc, x, y, prefix);
		data.loc.forEach(function (t) {
	...
	_action_turnBlock(data, x, y, prefix) {
		data.loc = this.__action_getLoc2D(data.loc, x, y, prefix);
		data.loc.forEach(function (t) {
	...
	_action_follow(data, x, y, prefix) {
		this.follow(data.name);
		core.doAction();
	}
	_action_unfollow(data, x, y, prefix) {
		this.unfollow(data.name);
		core.doAction();
	}
	_action_animate(data, x, y, prefix) {
		if (data.loc == 'hero') {
	...
	_action_stopAnimate(data, x, y, prefix) {
		core.stopAnimate(null, data.doCallback);
		core.doAction();
	}
	_action_setViewport(data, x, y, prefix) {
		if (data.dxy != null) {
	...
	_action_jumpHero(data, x, y, prefix) {
		var loc;
		if (data.dxy) {
	...
	_action_changeFloor(data, x, y, prefix) {
	...
	_action_changePos(data, x, y, prefix) {
		core.clearMap('hero');
	...
	_action_showImage(data, x, y, prefix) {
	...
	_precompile_showImage(data) {
		data.sloc = this.__precompile_array(data.sloc);
		data.loc = this.__precompile_array(data.loc);
		return data;
	}
	_action_showTextImage(data, x, y, prefix) {
	...
	_action_openDoor(data, x, y, prefix) {
		var loc = this.__action_getLoc(data.loc, x, y, prefix);
	...
	_action_loadEquip(data, x, y, prefix) {
		core.loadEquip(data.id);
		core.doAction();
	}
	_action_unloadEquip(data, x, y, prefix) {
		core.unloadEquip(data.pos);
		core.doAction();
	}
	_action_openShop(data, x, y, prefix) {
		core.setShopVisited(data.id, true);
		if (data.open) core.openShop(data.id, true);
		core.doAction();
	}
	_action_disableShop(data, x, y, prefix) {
		core.setShopVisited(data.id, false);
		core.doAction();
	}
	_action_battle(data, x, y, prefix) {
		if (data.id) {
	...
	_action_trigger(data, x, y, prefix) {
		var loc = this.__action_getLoc(data.loc, x, y, prefix);
		this._trigger_inAction(loc[0], loc[1]);
	}
	_action_insert(data, x, y, prefix) {
		if (data.name) { // 公共事件
	...
	_action_playBgm(data, x, y, prefix) {
		core.playBgm(data.name, data.startTime || 0);
		core.setFlag("__bgm__", data.keep ? data.name : null);
		core.doAction();
	}
	_action_pauseBgm(data, x, y, prefix) {
		core.pauseBgm();
		core.doAction();
	}
	_action_resumeBgm(data, x, y, prefix) {
		core.resumeBgm(data.resume);
		core.doAction();
	}
	_action_loadBgm(data, x, y, prefix) {
		core.loadBgm(data.name);
		core.doAction();
	}
	_action_freeBgm(data, x, y, prefix) {
		core.freeBgm(data.name);
		core.doAction();
	}
	_action_playSound(data, x, y, prefix) {
	...
	_action_stopSound(data, x, y, prefix) {
		core.stopSound();
		core.doAction();
	}
	_action_setVolume(data, x, y, prefix) {
	...
	_action_setBgmSpeed(data, x, y, prefix) {
		core.setBgmSpeed(data.value, data.pitch || false);
		core.doAction();
	}
	_action_setValue(data, x, y, prefix) {
	...
	_action_resetEnemyOnPoint(data, x, y, prefix) {
		var loc = this.__action_getLoc2D(data.loc, x, y, prefix);
		loc.forEach(function (one) {
	...
	_precompile_moveEnemyOnPoint(data) {
		data.from = this.__precompile_array(data.from);
		data.to = this.__precompile_array(data.to);
		data.dxy = this.__precompile_array(data.dxy);
		return data;
	}
	_action_moveEnemyOnPoint(data, x, y, prefix) {
		var from = this.__action_getLoc(data.from, x, y, prefix), to;
		if (data.dxy) {
			to = [from[0] + (core.calValue(data.dxy[0], prefix) || 0), from[1] + (core.calValue(data.dxy[1], prefix) || 0)];
		} else {
	...
	_action_input2(data, x, y, prefix) {
	...
	__action_getInput(hint, isText, callback) {
	...
	_action_switch(data, x, y, prefix) {
	...
	_action_choices(data, x, y, prefix) {
	...
	__action_choices_replaying(data, index) {
		var selection = index;
		if (index != 'none') {
	...
	_action_confirm(data, x, y, prefix) {
		data.text = core.replaceText(data.text, prefix);
	...
	__action_confirm_replaying(data, index) {
		if (index != 'none') {
	...
	_action_forEach(data, x, y, prefix) {
		// Only support temp:A
		if (!/^temp:[A-Z]$/.test(data.name)) {
			core.insertAction(['循环遍历事件只支持临时变量！']);
			return core.doAction();
	...
	_precompile_forEach(data) {
		data.data = this.precompile(data.data);
		return data;
	}
	_action_while(data, x, y, prefix) {
		if (core.calValue(data.condition, prefix)) {
	...
	_precompile_while(data) {
		data.condition = core.replaceValue(data.condition);
		data.data = this.precompile(data.data);
		return data;
	}
	_action_dowhile(data, x, y, prefix) {
		var list = core.clone(data.data);
		if (!(list instanceof Array)) list = [list];
		list.push({ "type": "_label" });
		core.unshift(core.status.event.data.list,
	...
	_precompile_dowhile(data) {
		data.condition = core.replaceValue(data.condition);
		data.data = this.precompile(data.data);
		return data;
	}
	_action_break(data, x, y, prefix) {
		var n = data.n || 1;
		while (n--) {
	...
	_action_continue(data, x, y, prefix) {
		var n = data.n || 1;
		while (n-- > 1) {
			if (core.status.event.data.list.length > 1)
				core.status.event.data.list.shift();
	...
	_action_win(data, x, y, prefix) {
		this.win(core.replaceText(data.reason, prefix), data.norank, data.noexit);
	}
	_action_lose(data, x, y, prefix) {
		this.lose(core.replaceText(data.reason, prefix));
	}
	_action_restart(data, x, y, prefix) {
		core.restart();
	}
	_action_function(data, x, y, prefix) {
	...
	_action_update(data, x, y, prefix) {
		core.updateStatusBar(data.doNotCheckAutoEvents, true);
		core.doAction();
	}
	_action_showStatusBar(data, x, y, prefix) {
		core.showStatusBar();
		core.doAction();
	}
	_action_hideStatusBar(data, x, y, prefix) {
		core.hideStatusBar(data.toolbox);
		core.doAction();
	}
	_action_showHero(data, x, y, prefix) {
		data.opacity = 1;
		return this._action_setHeroOpacity(data, x, y, prefix);
	}
	_action_hideHero(data, x, y, prefix) {
		data.opacity = 0;
		return this._action_setHeroOpacity(data, x, y, prefix);
	}
	_action_setHeroOpacity(data, x, y, prefix) {
		data.time = data.time || 0;
	...
	_action_waitAsync(data, x, y, prefix) {
	...
	_action_stopAsync(data, x, y, prefix) {
		core.stopAsync();
		core.doAction();
	}
	_action_callBook(data, x, y, prefix) {
	...
	_action_callSave(data, x, y, prefix) {
	...
	_action_autoSave(data, x, y, prefix) {
		var forbidSave = core.hasFlag('__forbidSave__');
		core.removeFlag('__forbidSave__');
	...
	_action_forbidSave(data, x, y, prefix) {
		core.setFlag('__forbidSave__', data.forbid || null);
		core.doAction();
	}
	_action_callLoad(data, x, y, prefix) {
		if (this.__action_checkReplaying()) return;
		var e = core.clone(core.status.event.data);
		core.ui.closePanel();
		core.load();
	...
	_action_exit(data, x, y, prefix) {
		this.setEvents([]);
		core.doAction();
	}
	_action_previewUI(data, x, y, prefix) {
		this.insertAction(data.action);
		core.doAction();
	}
	_precompile_previewUI(data) {
		data.action = this.precompile(data.action);
		return data;
	}
	__action_doUIEvent(data) {
	...
	_action_clearMap(data, x, y, prefix) {
		this.__action_doUIEvent(data);
	}
	_action_fillText(data, x, y, prefix) {
		data.text = core.replaceText(data.text, prefix);
		this.__action_doUIEvent(data);
	}
	_action_fillBoldText(data, x, y, prefix) {
		data.text = core.replaceText(data.text, prefix);
		this.__action_doUIEvent(data);
	}
	_action_fillRect(data, x, y, prefix) {
		this.__action_doUIEvent(data);
	}
	_action_fillPolygon(data, x, y, prefix) {
		this.__action_doUIEvent(data);
	}
	_precompile_fillPolygon(data) {
		data.nodes = this.__precompile_array(data.nodes);
		return data;
	}
	_action_strokeRect(data, x, y, prefix) {
		this.__action_doUIEvent(data);
	}
	_action_strokePolygon(data, x, y, prefix) {
		this.__action_doUIEvent(data);
	}
	_precompile_strokePolygon(data) {
		data.nodes = this.__precompile_array(data.nodes);
		return data;
	}
	_action_fillEllipse(data, x, y, prefix) {
		this.__action_doUIEvent(data);
	}
	_action_strokeEllipse(data, x, y, prefix) {
		this.__action_doUIEvent(data);
	}
	_action_fillArc(data, x, y, prefix) {
		this.__action_doUIEvent(data);
	}
	_action_strokeArc(data, x, y, prefix) {
		this.__action_doUIEvent(data);
	}
	_action_drawLine(data, x, y, prefix) {
		this.__action_doUIEvent(data);
	}
	_action_drawArrow(data, x, y, prefix) {
		this.__action_doUIEvent(data);
	}
	_action_setAttribute(data, x, y, prefix) {
		this.__action_doUIEvent(data);
	}
	_action_setFilter(data, x, y, prefix) {
		this.__action_doUIEvent(data);
	}
	_action_drawImage(data, x, y, prefix) {
		this.__action_doUIEvent(data);
	}
	_action_drawIcon(data, x, y, prefix) {
		this.__action_doUIEvent(data);
	}
	_action_drawSelector(data, x, y, prefix) {
		this.__action_doUIEvent(data);
	}
	_action_drawBackground(data, x, y, prefix) {
		this.__action_doUIEvent(data);
	}
	_action_drawTextContent(data, x, y, prefix) {
		data.text = core.replaceText(data.text, prefix);
		this.__action_doUIEvent(data);
	}
	// ------ 点击状态栏图标所进行的一些操作 ------ //
	////// 判断当前能否进入某个事件 //////
	_checkStatus(name, fromUserAction, checkItem) {
	...
	_updateValueByOperator(value, originValue, operator) {
		switch (operator) {
	...
	////// 数值操作 //////
	setValue(name, operator, value, prefix) {
	...
	////// 设置某个点上的怪物属性 //////
	setEnemyOnPoint(x, y, floorId, name, value, operator, prefix, norefresh) {
		floorId = floorId || core.status.floorId;
	...
	////// 重置某个点上的怪物属性 //////
	resetEnemyOnPoint(x, y, floorId, norefresh) {
		delete ((flags.enemyOnPoint || {})[floorId || core.status.floorId] || {})[x + "," + y];
		if (!norefresh) core.updateStatusBar();
	}
	////// 将某个点上已经设置的怪物属性移动到其他点 //////
	moveEnemyOnPoint(fromX, fromY, toX, toY, floorId, norefresh) {
		floorId = floorId || core.status.floorId;
	...
	////// 设置楼层属性 //////
	setFloorInfo(name, value, floorId, prefix) {
		floorId = floorId || core.status.floorId;
		core.status.maps[floorId][name] = value;
		core.updateStatusBar();
	}
	////// 设置全塔属性 //////
	setGlobalAttribute(name, value) {
		if (typeof value == 'string') {
	...
	moveTextBox(code, loc, relative, moveMode, time, callback) {
	...
	_moveTextBox_moving(ctx, moveInfo, callback) {
	...
	////// 显示图片 //////
	showImage(code, image, sloc, loc, opacityVal, time, callback) {
		var imageName = null;
	...
	////// 移动图片 //////
	moveImage(code, to, opacityVal, moveMode, time, callback) {
		to = to || [];
		var name = "image" + (code + 100);
		...
		var fromX = parseFloat(canvas.getAttribute("_left")), fromY = parseFloat(canvas.getAttribute("_top")), toX = getOrDefault(to[0], fromX), toY = getOrDefault(to[1], fromY);
	...
	_moveImage_moving(name, moveInfo, callback) {
		var per_time = 10, step = 0, steps = parseInt(moveInfo.time / per_time);
		if (steps <= 0) steps = 1;
		var fromX = moveInfo.fromX, fromY = moveInfo.fromY, toX = moveInfo.toX, toY = moveInfo.toY, opacity = moveInfo.opacity, toOpacity = moveInfo.toOpacity;
	...
	////// 旋转图片 //////
	rotateImage(code, center, angle, moveMode, time, callback) {
		center = center || [];
		var name = "image" + (code + 100);
		if (!core.dymCanvas[name]) {
			if (callback) callback();
	...
	_rotateImage_rotating(name, rotateInfo, callback) {
	...
	////// 放缩一张图片 //////
	scaleImage(code, center, scale, moveMode, time, callback) {
		center = center || [];
		var name = "image" + (code + 100);
		if (!core.dymCanvas[name]) {
			if (callback) callback();
	...
	_scaleInfo_scale(ctx, scaleInfo, scale) {
	...
	_scaleImage_scale(ctx, scaleInfo, callback) {
		if (!scaleInfo.time) {
	...
	////// 画面震动 //////
	vibrate(direction, time, speed, power, callback) {
		if (core.isReplaying()) {
			if (callback) callback();
			return;
		}
	...
	eventMoveHero(steps, time, callback) {
	...
	_eventMoveHero_moving(step, moveSteps) {
	...
	_jumpHero_doJump(jumpInfo, callback) {
		var cb = function () {
