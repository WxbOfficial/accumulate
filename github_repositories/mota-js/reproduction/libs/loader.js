...
export default class Loader {
	...
	////// 设置加载进度条进度 //////
	_setStartProgressVal(val) {
		core.dom.startTopProgress.style.width = val + '%';
	}
	////// 设置加载进度条提示文字 //////
	_setStartLoadTipText(text) {
	...
	_loadAutotiles_async(onprogress, onfinished) {
		core.material.images.autotile = {};
		var keys = Object.keys(core.material.icons.autotile);
		var autotiles = {};
	...
	_loadAutotiles_afterLoad(keys, autotiles) {
		// autotile需要保证顺序
	...
	_loadTilesets_async(onprogress, onfinished) {
		core.material.images.tilesets = {};
	...
	// ------ 实际加载一系列图片 ------ //
	loadImages(dir, names, toSave, callback) {
	...
	loadImage(dir, imgName, callback) {
		try {
			var name = imgName;
			if (name.indexOf(".") < 0)
				name = name + ".png";
	...
	// ------ 从zip中加载一系列图片 ------ //
	loadImagesFromZip(url, names, toSave, onprogress, onfinished) {
		if (!names || names.length == 0) {
	...
	_loadAnimates_async(onprogress, onfinished) {
	...
	_loadMusic_async(onprogress, onfinished) {
		core.bgms.forEach(function (t) {
			core.loader.loadOneMusic(t);
		});
	...
	_loadOneSound_decodeData(name, data) {
