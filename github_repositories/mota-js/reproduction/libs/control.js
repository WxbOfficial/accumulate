...
export default class Control {
	...
	_animationFrame_weather_rain(timestamp, level) {
	...
	_animationFrame_weather_snow(timestamp, level) {
		if (timestamp - core.animateFrame.weather.time < 30) return;
		var ctx = core.dymCanvas.weather, ox = core.bigmap.offsetX, oy = core.bigmap.offsetY;
		core.clearMap('weather');
	...
	__animateFrame_weather_image(timestamp, level) {
		if (timestamp - core.animateFrame.weather.time < 30) return;
	...
	_animationFrame_weather_sun(timestamp, level) {
		if (timestamp - core.animateFrame.weather.time < 30) return;
		var node = core.animateFrame.weather.nodes[0];
	...
	////// 清空剩下的自动寻路列表 //////
	clearContinueAutomaticRoute(callback) {
		core.deleteCanvas('route');
		core.status.automaticRoute.moveStepBeforeStop = [];
		if (callback) callback();
	}
	////// 设置自动寻路路线 //////
	setAutomaticRoute(destX, destY, stepPostfix) {
	...
	_setAutomaticRoute_isMoving(destX, destY) {
	...
	_setAutomaticRoute_isTurning(destX, destY, stepPostfix) {
	...
	_setAutomaticRoute_clickMoveDirectly(destX, destY, stepPostfix) {
		// 单击瞬间移动
	...
	_setAutomaticRoute_drawRoute(moveStep) {
	...
	_setAutomaticRoute_setAutoSteps(moveStep) {
		// 路线转autoStepRoutes
	...
	_moveAction_noPass(canMove, callback) {
	...
	////// 瞬间移动 //////
	moveDirectly(destX, destY, ignoreSteps) {
		return this.controldata.moveDirectly(destX, destY, ignoreSteps);
	}
	////// 尝试瞬间移动 //////
	tryMoveDirectly(destX, destY) {
	...
	_drawHero_updateViewport(x, y, offset) {
	...
	_drawHero_draw(direction, x, y, status, offset, frame) {
	...
	_drawHero_getDrawObjs(direction, x, y, status, offset) {
	...
	setHeroOpacity(opacity, moveMode, time, callback) {
		time = time || 0;
		if (time == 0) {
	...
	// ------ 画布、位置、阻激夹域，显伤 ------ //
	////// 设置画布偏移
	setGameCanvasTranslate(canvas, x, y) {
	...
	////// 移动视野范围 //////
	moveViewport(x, y, moveMode, time, callback) {
		time = time || 0;
		time /= Math.max(core.status.replay.speed, 1);
	...
	_updateDamage_damage(floorId, onMap) {
	...
	_updateDamage_extraDamage(floorId, onMap) {
	...
	_replayAction_getNext(action) {
		if (action != "getNext") return false;
		core.getNextItem();
		core.replay();
		return true;
	}
	_replayAction_moveDirectly(action) {
	...
	_replayAction_ignoreInput(action) {
	...
	_doSL_replayLoad_afterGet(id, data) {
		if (!data) {
			core.playSound('操作失败');
	...
	_doSL_replayRemain_afterGet(id, data) {
		if (!data) {
			core.playSound('操作失败');
			return core.drawTip("无效的存档");
		}
	...
	_doSL_replaySince_afterGet(id, data) {
	...
	_getSaveIndexes_getIndex(indexes, name) {
	...
	getStatusOrDefault(status, name) {
	...
	getRealStatusOrDefault(status, name) {
	...
	////// 获得或移除毒衰咒效果 //////
	triggerDebuff(action, type) {
		return this.controldata.triggerDebuff(action, type);
	}
	////// 设置勇士的位置 //////
	setHeroLoc(name, value, noGather) {
		if (!core.status.hero) return;
	...
	////// 更改画面色调 //////
	setCurtain(color, time, moveMode, callback) {
	...
	_setCurtain_animate(nowColor, color, time, moveMode, callback) {
		time /= Math.max(core.status.replay.speed, 1);
		var per_time = 10, step = 0, steps = parseInt(time / per_time);
		if (steps <= 0) steps = 1;
		var curr = nowColor;
	...
	////// 画面闪烁 //////
	screenFlash(color, time, times, moveMode, callback) {
		times = times || 1;
		time = time / 3;
	...
	////// 播放音频 //////
	playSound(sound, pitch, callback) {
	...
	////// 更新状态栏 //////
	updateStatusBar(doNotCheckAutoEvents, immediate) {
		if (!core.isPlaying()) return;
		if (immediate) {
	...
	_updateStatusBar_setToolboxIcon() {
		if (core.isReplaying()) {
